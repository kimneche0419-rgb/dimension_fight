...
class GameManager:
    ...
    def _replace_enemies_on_universe_shift(self):
        """멀티버스 전환 시 비-보스 적 전부 교체"""
        boss_types = {"nexus_overmind","abyssal_tyrant","void_god","abyss_sovereign",
                      "echo_wraith","dreadnought_construct","anomaly_core",
                      "abyss_leviathan","abyss_hydra","null_colossus"}
        ...
        universe_pools = {
            "PRIME":   normal_pool,
            "CYBER":   ["glitcher","sniper_node","hunter_drone","elite_enforcer","corrupted_sentry"],
            "ABYSSAL": abyss_pool + ["shadow_lurker","void_weaver"],
            "GOLDEN":  normal_pool + ["swarm_organism","basic_drone"],
            "GLITCH":  void_pool + ["glitcher","corrupted_sentry","shadow_lurker"],
        }
        
        pool = universe_pools.get(self.universe_type, normal_pool)
        if ch.enemy_set == "abyss":
            pool = pool + abyss_pool
        elif ch.enemy_set == "void":
            pool = pool + void_pool
        elif ch.enemy_set == "all":
            pool = pool + abyss_pool + void_pool
        pool = list(set(pool))  # 중복 제거
        
        replaceable = [e for e in self.enemies if e.etype not in boss_types]
        for enemy in replaceable:
            pos = Vector2(enemy.world_pos)
            dim = random.choice(["PHYSICAL","VOID"])
            etype = random.choice(pool)
            self._burst(enemy.world_pos, self.universes[self.universe_type]["color"], count=6, speed=3)
            enemy.kill()
            new_e = Enemy(pos, dim, etype, self.difficulty)
    ...
    def _shift_universe(self, target=None):
        ...
        # ★ 멀티버스 전환 시 비-보스 적 전부 교체
        self._replace_enemies_on_universe_shift()
    ...
    def _draw_playing(self, shake):
        ...
        self.draw_text(f"ENTROPY: {self.entropy*100:.1f}%", (10, 125), 14, e_color, align="left")
