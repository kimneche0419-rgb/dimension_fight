...
class GameManager:
    ...
    def __init__(self, screen):
        ...
        self.chapters = {
            "0": Chapter(
                "전직의 시련 (1회 한정)", "SHIP", -0.05,
                [(0.0,(40,10,20)),(1.0,(180,20,50))],
                60, "전직의 자격을 증명하라 · 1분 생존 필수 · 심연 위협",
                enemy_set="abyss", special=["diving", "abyss_enemies", "mega_bosses"]
            ),
            "1": Chapter(
    ...
    def update(self, events):
        ...
        # 레벨 보너스: 레벨당 난이도 1.5 증가 (매우 빠른 난이도 상승)
        level_bonus = (self.player.level - 1) * 1.5
        # 후반부 폭증 커브 (엄청난 난이도 상승폭)
        self.difficulty = 1.0 + (progress ** 1.1) * 25.0 + level_bonus
        ...
        self.spawn_timer += 1
        interval = max(1, int(15 - self.difficulty * 4.0))  # ↑ 훨씬 빠른 스폰, 최소 1프레임
    ...
    def _handle_menu_input(self, events):
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                # 챕터 카드 직접 클릭
                chapter_list = []
                if not getattr(self, "played_job_chapter", False) and "0" in self.chapters:
                    chapter_list.append("0")
                chapter_list.extend([str(_i) for _i in range(1, 7)])

                cols = 2 if len(chapter_list) <= 6 else 3
                if cols == 3:
                    card_w, card_h = 240, 88
                    x_start, y_start, x_gap, y_gap = 30, 140, 250, 108
                else:
    ...
    def _handle_shop_mouse(self, event):
        ...
        if event.type == pygame.MOUSEWHEEL:
            ...
            rows = (len(all_items) + 1) // 2 + 3 # 섹션 타이틀 공간 포함
        ...
        y_cursor = 160 + self.shop_scroll_y
    ...
    def _draw_menu(self):
        ...
        clist = []
        if not getattr(self, "played_job_chapter", False) and "0" in self.chapters:
            clist.append(("0", self.chapters["0"]))
        for cid in ["1", "2", "3", "4", "5", "6"]:
            if cid in self.chapters: clist.append((cid, self.chapters[cid]))

        cols = 2 if len(clist) <= 6 else 3
        if cols == 3:
            card_w, card_h = 240, 88
            x_start, y_start, x_gap, y_gap = 30, 140, 250, 108
        else:
        ...
        self.draw_text("WASD이동  SPACE대쉬  SHIFT차원전환  F변형  Q/E무기  [1~6]스킬  [S]상점  [I]스킬관리", (400, 555), 13, (100, 100, 150))
    ...
    def _draw_hud(self, progress):
        ...
        if self.player.job:
            ...
            self.draw_text(f"[{self.player.job}]", (88, 127), 13, jcol, align="left")
    ...
    def _draw_job_select_overlay(self):
        ...
        for i, jkey in enumerate(self.job_select_choices):
            ...
            # ── 직업 이름 ──
            name_y = cy + 60
            self.draw_text(jd["name"], (cx + card_w // 2, name_y), 24, jd["color"])

            # ── 설명 ──
            self.draw_text(jd["desc"], (cx + card_w // 2, name_y + 26), 11, (190, 195, 220))
            ...
            # ── 버프 섹션 ──
            self.draw_text("▲ BUFF", (cx + card_w // 2, line_y + 14), 11, (80, 230, 120))
            buff_lines = jd["buff"].split(" · ")
            for li, line in enumerate(buff_lines[:2]):
                self.draw_text(line, (cx + card_w // 2, line_y + 30 + li * 15), 10, (150, 235, 175))

            # ── 너프 섹션 ──
            nerf_y = line_y + 70
            self.draw_text("▼ NERF", (cx + card_w // 2, nerf_y), 11, (230, 80, 80))
            self.draw_text(jd["nerf"], (cx + card_w // 2, nerf_y + 15), 10, (240, 160, 160))
